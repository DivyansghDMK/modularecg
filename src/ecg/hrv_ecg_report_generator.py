    ...
    for segment_idx in range(num_segments):
        ...
        try:
            ...
            if len(segment_data) > 0:
                ...
                # DEBUG: Check if data is already processed (SAME AS MAIN REPORT)
                data_mean = np.mean(adc_data)
                data_std = np.std(adc_data)
                
                # Step 2: Apply baseline 2000 (EXACT LOGIC FROM MAIN REPORT line 2668-2688)
                # IMPORTANT: For Lead II, data is usually raw ADC around baseline 2000
                is_calculated_lead = False  # Lead II is NOT a calculated lead
                
                if abs(data_mean - 2000.0) < 500:  # Data is close to baseline 2000 (raw ADC)
                    centered_adc = adc_data - baseline_adc
                elif is_calculated_lead:
                    # Not applicable for Lead II
